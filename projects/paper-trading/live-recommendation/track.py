...
portfolio/FINAL_PORTFOLIO_RECOMMENDATION.md Section 3. Whenever that table's holdings
or weights change (the doc revises roughly every 2 days), this script rebalances to
the new target weights at live prices -- so it answers "what would Rs 1L invested
exactly as recommended, kept exactly in sync, be worth today?" as opposed to the
frozen cohorts' "what would Rs 1L invested last Monday be worth?"
    ...
    if pf is None:
        ...
        pf = {
            "_readme": "LIVE recommendation tracker. SEPARATE from paper-trading/cohorts.json (frozen weekly "
                       ...
                       "always mirrors the CURRENT allocation in portfolio/FINAL_PORTFOLIO_RECOMMENDATION.md "
                       "Section 3, auto-rebalancing at live prices whenever that table's holdings or weights "
                       "change by more than a rounding amount. Script-owned -- do not hand-edit holdings/cash; "
                       "edit the recommendation doc instead, this script picks it up on the next run.",
...
def write_tracker(pf, snap, hist):
    ...
    a(f"- Always mirrors `portfolio/FINAL_PORTFOLIO_RECOMMENDATION.md` Section 3 -- rebalances at live prices "
      f"whenever a holding is added/dropped or a weight moves more than {REBALANCE_THRESHOLD_PP}pp. "

#!/usr/bin/env python3
"""
paper-trading/live-recommendation/track.py -- daily mark-to-market AND auto-rebalance
for the LIVE recommendation tracker.

Unlike the frozen weekly cohorts (paper-trading/cohorts.json) or the actively-managed
swing cohorts (swing-6m/cohorts.json), this is a single, continuously-rebalanced
Rs 1,00,000 paper portfolio that always mirrors the CURRENT allocation in
valuepickr-open-screen/data/confluence100_allocation.json (2026-09-22 -- moved off
portfolio/FINAL_PORTFOLIO_RECOMMENDATION.md, whose source job, portfolio-rs1l-revision,
is pending retirement). Whenever that allocation's holdings or weights change (rebuilt
weekly by vpscreen-rerank Step 7.2), this script rebalances to the new target weights at
live prices -- so it answers "what would Rs 1L invested exactly as recommended, kept
exactly in sync, be worth today?" as opposed to the frozen cohorts' "what would Rs 1L
invested last Monday be worth?"
...
def main():
    ...
    target_weights, revision = rf.parse_recommendation_weights()
    ...
    if pf is None:
        ...
        pf = {
            "_readme": "LIVE recommendation tracker. SEPARATE from paper-trading/cohorts.json (frozen weekly "
                       "cohorts) and swing-6m/cohorts.json (actively-managed swing cohorts). This single book "
                       "always mirrors the CURRENT allocation in "
                       "valuepickr-open-screen/data/confluence100_allocation.json (2026-09-22 on), "
                       "auto-rebalancing at live prices whenever that allocation's holdings or weights "
                       "change by more than a rounding amount. Script-owned -- do not hand-edit holdings/cash; "
                       "it picks up the next weekly Confluence-100 allocation rebuild automatically.",
    ...
    hist = json.load(open(HIST_PATH)) if os.path.exists(HIST_PATH) else {
        "_readme": "Append-only daily NAV snapshots for the live recommendation tracker. One per date, "
    ...
    print(f"live-recommendation track -- {today} ({datetime.datetime.now():%Y-%m-%d %H:%M})  [v{pf['version']}]")
    print("-" * 64)
...
def write_tracker(pf, snap, hist):
    L = []
    a = L.append
    a("# Live Recommendation Tracker -- Returns\n")
    a(f"_Regenerated by `paper-trading/live-recommendation/track.py` on {snap['date']}. Do not hand-edit._\n")
    ...
    a(f"- Always mirrors `valuepickr-open-screen/data/confluence100_allocation.json` -- rebalances at live "
      f"prices whenever a holding is added/dropped or a weight moves more than {REBALANCE_THRESHOLD_PP}pp. "
      f"Source: {pf.get('source_revision', '?')}.")
    a("- SEPARATE from `paper-trading/cohorts.json` (frozen weekly cohorts, never rebalanced) and "
      "`paper-trading/swing-6m/cohorts.json` (actively-managed swing cohorts).\n")
